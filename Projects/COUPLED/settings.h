...
#define DOMAIN_BITS 8 //10 < Bits for domain resolution. 8 -> 2^8x2^8x2^8 grid-nodes. Increase for more grid-nodes.
...
constexpr int g_max_active_block = 25000; //25000 5000 < Max active blocks in gridBlocks. Preallocated, can resize. Lower = less memory used.
/// 62500 bytes for active mask

// * Particles
#define MAX_PPC 32 //64 < VERY important. Max particles-per-cell. Substantially effects memory/performance, exceeding MAX_PPC deletes particles. Generally, use MAX_PPC = 8*(Actualy PPC) to account for compression.
...
// * Halo Blocks
constexpr std::size_t g_max_halo_block = 0;  //1024 < Max active halo blocks. Preallocated, can resize.
