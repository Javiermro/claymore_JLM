...
/// * Simple U-P formulations - Stress and Energy (JLM)
/// * Elasto-plastic model. Good for snow, clay, concrete, etc.
template <typename T = float>
__forceinline__ __device__ void
compute_stress_coupledup(T volume, T mu, T lambda, T bm, T xi, T beta, T Msqr,
                    bool hardeningOn, T &logJp, vec<T, 9> &F, vec<T, 9> &PF) {
  T U[9], S[3], V[9];
  math::svd(F[0], F[3], F[6], F[1], F[4], F[7], F[2], F[5], F[8], U[0], U[3],
            U[6], U[1], U[4], U[7], U[2], U[5], U[8], S[0], S[1], S[2], V[0],
...
template <>
__forceinline__ __device__ void
compute_stress_coupledup(double volume, double mu, double lambda, double bm, double xi, double beta, double Msqr,
                    bool hardeningOn, double &logJp, vec<double, 9> &F, vec<double, 9> &PF) {
  using T = double;
  T U[9], S[3], V[9];
  math::svd(F[0], F[3], F[6], F[1], F[4], F[7], F[2], F[5], F[8], U[0], U[3],
